<div class="d-flex align-items-center">
    ...
    {{-- @if(isset($plural))
        @if(is_countable($plural))
            @if(count($plural) > 1) --}}
                <div class="dropdown">
                    <button class="btn dropdown-toggle hide-arrow text-body p-0" id="dropdown-{{ $collection->id }}" data-bs-auto-close="outside"  data-bs-toggle="dropdown"  aria-expanded="false">
            ...
            {{-- @else
                <button class="btn btn-sm btn-icon btn-popover" type="button" >
                    <i class="bx bx-dots-vertical-rounded"></i>
                </button>
            @endif
        @endif
