            ...
            <div class="card mb-4" id="{{ rand() }}">
                <div class="card-body">
                    <div class="row">
                        <div class="col-12">
                            <dl class="row mb-0">
                                <dt class="col-sm-5 fw-semibold mb-3 text-nowrap">Extension:</dt>
                                <dd class="col-sm-7 text-wrap">{{ setDate($extensionModel->extension) }}</dd>

                                <dt class="col-sm-5 fw-semibold mb-3 text-nowrap">Date From:</dt>
                                <dd class="col-sm-7 text-wrap">{{ $extensionModel->date_from }}</dd>

                                <dt class="col-sm-5 fw-semibold mb-3 text-nowrap">Date To</dt>
                                <dd class="col-sm-7 text-wrap">{{ $extensionModel->date_to  }}</dd>

                                <dt class="col-sm-5 fw-semibold mb-3 text-nowrap">Quantity</dt>
                                <dd class="col-sm-7 text-wrap">{{ $extensionModel->quantity }}</dd>

                                <dt class="col-sm-5 fw-semibold mb-3 text-nowrap">Benefeciaries</dt>
                                <dd class="col-sm-7 text-wrap">{{ $extensionModel->beneficiaries }}</dd>

                                <dt class="col-sm-5 fw-semibold mb-3 text-nowrap"><i class='bx bx-comment me-1'></i> Evalution remark</dt>
                                <dd class="col-sm-7">

                                    <label class="switch switch-square" wire:click.prevent="remark('{{ ($extensionModel->is_evaluated == 0 ? 1 : 0) }}')">
                                        <input type="checkbox" class="switch-input" {{ ($extensionModel->is_evaluated == 1 ? 'checked' : null) }}>
                                        ...
                                        @if($extensionModel->is_evaluated  == 0)
        ...
        <div class="col-md-7 col-12">
            ...
            <div class="card">
                ...
                <div class="card-body">
                    ...
                    <ul class="timeline" >
                        @if(isset($extensionModel->evaluations))
                            @if(count($extensionModel->evaluations) > 0)

                                @foreach($extensionModel->evaluations as $evaluation)
                                ...
                                @include('vendor.remis-components.evaluation-starter-timeline',[
                                    'title' => $extensionModel->title,
                                    'dateCreated' => $extensionModel->date_created,
                                    'owner' => ['firstname' => $extensionModel->file_owner->firstname, 'lastname' => $extensionModel->file_owner->lastname],
                                    'attachments' => $extensionModel->attachments
                                ])
